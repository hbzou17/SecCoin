...
static const int64 nNewTargetTimespan = 60 * 60; // 3.5 days
static const int64 nTargetTimespan = 60 * 60; // two weeks
static const int64 nTargetSpacing = 315;
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, uint256 preblockhash, int nblockversion,  uint256 nNonce)
{
    ...
    printf("CheckProofOfWork preblockhash: %s\n", preblockhash.ToString().c_str());
    printf("CheckProofOfWork nblockversion: %d\n", nblockversion);
    printf("CheckProofOfWork nNonce: %s\n", nNonce.ToString().c_str());
    if (0 == preblockhash);
        return true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain) {
            ...
            return error("AardvarkCoinMiner : generated block is stale");
        }
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AardvarkCoinMiner : ProcessBlock, block not accepted");
    }
...
void static AardvarkCoinMiner(CWallet *pwallet)
{
    printf("AardvarkCoinMiner started\n");
    ...
    try { while(true) {
        ...
        printf("Running AardvarkCoinMiner with %" PRIszu " transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AardvarkCoinMiner terminated\n");
        throw;
    }
}

void GenerateAardvarkCoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AardvarkCoinMiner, pwallet));
}
