...
Value importkey(const Array& params, bool fHelp)
{
    ...
    CKeyID vchAddress = key.GetPubKey().GetID();
    {
        ...
        if(vArgs.size() > 2) {
            ...
            if (!pwalletMain->AddPubKeyPos(CAardvarkCoinAddress(vchAddress).ToString(), pos))
...
Value dumpkey(const Array& params, bool fHelp)
{
    ...
    if (pwalletMain->GetPubKeyPos(strAddress, pos))
        return CAardvarkCoinSecret(vchSecret).ToString()+"|" + EncodeBase58(pubKey.vchPubKey) + "|" + HexStr(pos.ToVector());
    else
        return CAardvarkCoinSecret(vchSecret).ToString()+"|" + EncodeBase58(pubKey.vchPubKey);
