...
const int ARKMINT_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString ARKMINT_IPC_PREFIX("abcmint:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(ARKMINT_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(ARKMINT_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(ARKMINT_IPC_CONNECT_TIMEOUT);
